...
Currently, we have implemented the spherical blob, which simply consists of a
centre point, an \fff, and an iso value where the surface is defined.

The \fff we have implemented is the Geoff function: $r$ is the distance from
the central point, $R$ is the maximum effective distance of the \fff.
$$f(r) = 1 - {4\over9} {r^6 \over R^6} + {17 \over 9} {r^4 \over R ^ 4} -
{22\over 9} {r^2 \over R^2}$$

I have only implemented one primitive object, the spherical blob. The primitive
node will always be a leaf node on the blob-tree, halting further traversal.
Evaluating the field function and normal of the primitive object is simple:
...
the point of intersection of a ray with the AABB \cite{Williams}.
